(function (root, factory) {
  ...
  function assertIsOfType$lambda(closure$message, closure$type, closure$value) {
    return function () {
      var tmp$ = messagePrefix(closure$message);
      var tmp$_0 = 'Expected value to be of type <' + closure$type + '>, actual <';
      if (closure$value != null) {
        var tmp$_1 = Kotlin.getKClassFromExpression(closure$value);
      } else
        tmp$_1 = null;
      return tmp$ + (tmp$_0 + toString(tmp$_1) + '>.');
    };
  }
  ...
  function assertContentEquals_0(expected, actual, message) {
    ...
    if (actual != null) {
      ...
      tmp$ = null;
    assertContentEquals(expected, tmp$, message);
  }
  ...
  function checkResultIsFailure(message, blockResult) {
    ...
    if (exception == null) {
      ...
      var tmp$_0 = get_asserter().fail_pdl1vj$(messagePrefix(message) + 'Expected an exception to be thrown, but was completed successfully.');
    } else {
      return exception;
    }
  }
  ...
  function checkResultIsFailure_0(exceptionClass, message, blockResult) {
    ...
    if (exception == null) {
      (tmp$ = blockResult.value) == null || Kotlin.isType(tmp$, Any) || throwCCE();
      var tmp$_0 = get_asserter().fail_pdl1vj$(messagePrefix(message) + ('Expected an exception of ' + exceptionClass + ' to be thrown, but was completed successfully.'));
    } else {
      var tmp$_1;
  ...
  function detectAdapter() {
    ...
    if (!equals(typeof kotlinTest, 'undefined')) {
      ...
      if (adapterTransform !== null) {
        var tmp$_0 = adapterTransform(frameworkAdapter);
      } else
